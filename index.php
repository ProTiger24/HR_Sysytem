                    ...
                    <?php if(isset($_SESSION['user_id'])): ?>
                        <li class="nav-item">
                            <a class="nav-link btn btn-primary text-white px-4 ms-2" 
                               href="<?php echo $_SESSION['user_type'] == 'hr' ? 'hr-dashboard.php' : 'employee-dashboard.php'; ?>">
                                <i class="fas fa-tachometer-alt me-2"></i>Dashboard
                            </a>
                        </li>
                    <?php else: ?>
                        <li class="nav-item">
                            <a class="nav-link btn btn-primary text-white px-4 ms-2" href="login.php">
            ...
            <div class="row align-items-center">
                ...
                <div class="col-lg-6 hero-image mt-5 mt-lg-0 fade-in-up">
                    <img src="https://images.unsplash.com/photo-1552664730-d307ca884978?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80" 
                         alt="Professional HR Dashboard Interface" 
                         style="border-radius:20px; width:100%; height:auto; box-shadow: 0 30px 60px rgba(0,0,0,0.3);">
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="stats-section">
        <div class="container">
            <div class="row text-center">
                ...
                <div class="col-md-3 col-6 stat-item">
                    <div class="stat-number" data-count="50" data-suffix="K+">0K+</div>
                    <div class="stat-label">Employees Managed</div>
                </div>
                <div class="col-md-3 col-6 stat-item">
                    <div class="stat-number" data-count="99.9" data-suffix="%">0%</div>
            ...
            <div class="row align-items-center">
                <div class="col-lg-6 about-image mb-5 mb-lg-0">
                    <img src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80" 
                         alt="KormoShathi Team" 
                         style="border-radius:20px; width:100%; height:auto; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
                </div>
    ...
    <script>
        ...
        async function loadRealtimeStats() {
            try {
                ...
                if (data.success) {
                    updateStatNumbersWithRealtimeData(data.stats);
                    return true;
                }
                return false;
            } catch (error) {
                console.log('API not ready, using default animation');
                return false;
            }
        }

        function updateStatNumbersWithRealtimeData(stats) {
            ...
            counters.forEach(counter => {
                ...
                let suffix = '';
                if (dataCount === '500') {
                    targetValue = stats.total_companies || 500;
                    suffix = '+';
                } else if (dataCount === '50') {
                    targetValue = stats.total_employees || 50;
                    suffix = 'K+';
                } else if (dataCount === '99.9') {
                    targetValue = stats.uptime || 99.9;
                    suffix = '%';
                } else if (dataCount === '24') {
                    targetValue = stats.support_hours || 24;
                    suffix = '/7';
                }
                animateSingleCounter(counter, targetValue, suffix);
            });
        }
        ...
        const statsObserver = new IntersectionObserver(async (entries) => {
            for (const entry of entries) {
                if (entry.isIntersecting) {
                    const realtimeLoaded = await loadRealtimeStats();
                    if (!realtimeLoaded) {
